...
public class EventManager {
     ...
     * @return Foglio Riepilogativo creato
     */
    public SummarySheet createSummarySheet(Event e) {
        ...
        if (e.getOwner() != u) throw new UseCaseLogicException("Lo chef non è proprietario dell'evento");
        else {
            currentEvent = e;
            SummarySheet s = new SummarySheet(u);
            currentEvent.setSummarySheet(s);
    ...
    public void deleteSummarySheet(Event e)
    {
        ...
        if (!u.isChef()) throw new UseCaseLogicException("Solo gli chef possono eliminare un foglio riepilogativo");
            ...
            if(e.getSummarySheet() != null) {
                for (EventReceiver r: receivers) {
                    r.notifySummarySheetDeleted(currentEvent);
                }
                e.setSummarySheet(null);
            }
        }
    }
    ...
    public Job addJob(Recipe rec) {
        ...
        //Creazione job
        Job j = new Job(rec);
        //Inserimento nel foglio riepilogativo
        this.currentEvent.getSummarySheet().addJob(j);
     ...
     */
    public void removeAllJobs(Recipe rec) {
        if(currentEvent == null) throw new UseCaseLogicException("non è stato specificato il foglio riepilogativo su cui si sta lavorando");
        
        this.currentEvent.getSummarySheet().removeAllJobs(rec);
        for (EventReceiver r: receivers) {
    ...
    public void removeJob(Job j) {
        ...
        boolean removed = this.currentEvent.getSummarySheet().removeJob(j);
        if(removed)
        {
            for (EventReceiver r: receivers) {
    ...
    public void moveJob(Job job, int pos) {
        ...
        if (sheet == null || !sheet.hasJob(job)) 
            throw new UseCaseLogicException("il compito " + job.toString() + " non appartiene al foglio riepilogativo dell'evento corrente");
    ...
    public void assignJob(Job j, Shift s, User cook, int eval, double portions, boolean completed, LocalDate date) {
        ...
        j.setCook(cook);
        j.setEval(eval);
        j.setPortions(portions);
        j.setCompleted(completed);
